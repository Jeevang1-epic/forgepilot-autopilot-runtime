...
const statusTone: Record<TimelineStepStatus, string> = {
  pending: "border-white/10 bg-white/[0.05] text-white/56",
  ...
  approval_required: "border-amber-200/30 bg-amber-200/10 text-amber-100",
};
...
export function TimelineStepCard({ step, index }: TimelineStepCardProps) {
  return (
    <article className="relative rounded-lg border border-white/10 bg-white/[0.045] p-5 shadow-xl shadow-black/20">
      <div className="absolute -left-[2.15rem] top-6 hidden h-4 w-4 rounded-full border border-teal-200/60 bg-[#06110f] shadow-[0_0_0_6px_rgba(45,212,191,0.08)] lg:block" />

      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="font-mono text-xs text-white/34">STEP {String(index + 1).padStart(2, "0")}</p>
      ...
      <div className="mt-5 grid gap-3 border-t border-white/10 pt-4 sm:grid-cols-2">
        <div>
          <p className="text-xs uppercase tracking-[0.18em] text-white/34">Timestamp</p>
          <p className="mt-2 font-mono text-sm text-white/72">{formatTime(step.timestamp)} UTC</p>
        </div>
        <div>
          <p className="text-xs uppercase tracking-[0.18em] text-white/34">Tool</p>
          <p className="mt-2 font-mono text-sm text-white/72">{step.toolName ?? "runtime.event"}</p>
        </div>
      </div>
    </article>
  );
