...
export function ArtifactPanel({ artifacts }: ArtifactPanelProps) {
  return (
    <aside className="rounded-lg border border-white/10 bg-white/[0.045] p-5 shadow-2xl shadow-black/25 xl:sticky xl:top-28">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="rounded-full border border-white/10 bg-black/30 px-2.5 py-1 text-xs text-white/54">
      ...
      <div className="mt-5 space-y-3">
        {artifacts.map((artifact) => (
          <div key={artifact.id} className="rounded-lg border border-white/10 bg-black/20 p-4">
            <div className="flex items-start justify-between gap-3">
              <div>
                <p className="font-mono text-sm font-semibold text-white">{artifact.fileName}</p>
                <p className="mt-2 text-xs leading-5 text-white/48">{artifact.description}</p>
              </div>
            ...
            <div className="mt-4 flex items-center justify-between border-t border-white/10 pt-3">
              <span className="font-mono text-xs text-white/36">{artifact.path}</span>
              <span className="font-mono text-xs text-white/50">{artifact.size}</span>
            </div>
          </div>
        ))}
      </div>
